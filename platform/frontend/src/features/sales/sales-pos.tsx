...
import { usePostSalesInvoice } from "./api";
...
export function SalesPOS() {
  ...
  const mutation = usePostSalesInvoice();
  ...
  const submit = () => {
    if (!invoiceNo || !companyId || !branchId || !customerId || lines.length === 0 || mutation.isPending) return;
    mutation.mutate({
  ...
  }, [invoiceNo, companyId, branchId, customerId, lines, mutation.isPending]);
  ...
  return (
    <section className="p-3 border border-slate-800 rounded bg-slate-900 space-y-2">
      <h2 className="text-sm font-semibold">Sales POS (Ctrl+Enter save, Alt+N add row, Alt+D delete row)</h2>
      <div className="grid grid-cols-3 gap-2">
        ...
        <input value={customerId} onChange={(e) => setCustomerId(e.target.value)} className="bg-slate-950 border border-slate-700 rounded px-2 py-1" placeholder="Customer ID" />
      </div>
