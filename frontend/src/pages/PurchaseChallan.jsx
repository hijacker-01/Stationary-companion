...
export default function PurchaseChallan() {
  ...
  const fetchChallans = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`${API}/api/purchase-challan`, { headers: headers() });
  ...
  const resetForm = () => {
    setForm({ supplierName: "", date: new Date().toISOString().slice(0, 10), notes: "" });
    setItems([emptyItem()]);
    setShowModal(false);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post(`${API}/api/purchase-challan`, payload, { headers: headers() });
      showToast("Challan created successfully");
      resetForm();
      fetchChallans();
    } catch (e) {
      showToast("Error creating challan");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleApprove = async (id) => {
    ...
    try {
      await axios.post(`${API}/api/purchase-challan/${id}/approve`, {}, { headers: headers() });
      showToast("Challan approved");
      fetchChallans();
    } catch (e) {
      showToast("Error approving challan");
    }
  };
  ...
  const handleConvert = async (id) => {
    ...
    if (!invoiceNo) return;
    try {
      await axios.post(`${API}/api/purchase-challan/${id}/convert`, { invoiceNo }, { headers: headers() });
      showToast("Converted to bill");
      fetchChallans();
    } catch (e) {
      showToast("Error converting challan");
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API}/api/purchase-challan/${id}`, { headers: headers() });
      showToast("Challan deleted");
      fetchChallans();
    } catch (e) {
      showToast("Error deleting challan");
    }
  };

  const handleView = async (id) => {
    try {
      const { data } = await axios.get(`${API}/api/purchase-challan/${id}`, { headers: headers() });
      setViewModal(data.data || data);
    } catch (e) {
      showToast("Error loading challan details");
    }
  };
                ...
                challans.map((c) => {
                  ...
                  return (
                    <tr key={c._id} className="border-b hover:bg-gray-50">
                      <td className="px-3 py-1.5 font-mono">{c.challanNo || c._id?.slice(-6)}</td>
                      <td className="px-3 py-1.5">{c.supplierName}</td>
                      ...
                      <td className="px-3 py-1.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {(status === "draft" || status === "pending") && (
                            <button onClick={() => handleApprove(c._id)} className="p-1 hover:bg-green-50 rounded" title="Approve">
                              <CheckCircle size={13} className="text-green-600" />
                            </button>
                          )}
                          {status === "approved" && (
                            <button onClick={() => handleConvert(c._id)} className="p-1 hover:bg-blue-50 rounded" title="Convert to Bill">
                              <ArrowRightCircle size={13} className="text-blue-600" />
                            </button>
                          )}
                          {status === "draft" && (
        ...
        <div className="fixed inset-0 z-40 bg-black/40 flex items-start justify-center pt-10 overflow-auto">
          <div className="bg-white rounded shadow-lg w-full max-w-3xl m-4" onKeyDown={handleKeyDown} tabIndex={-1}>
            <div className="flex items-center justify-between px-4 py-2 border-b bg-gray-50">
              <span className="text-sm font-semibold">Challan Details</span>
            ...
            <div className="p-4 text-xs space-y-3">
              <div className="grid grid-cols-4 gap-3">
                <div><span className="text-gray-500">Challan No:</span> <span className="font-semibold">{viewModal.challanNo || viewModal._id?.slice(-6)}</span></div>
                <div><span className="text-gray-500">Supplier:</span> <span className="font-semibold">{viewModal.supplierName}</span></div>
                ...
                <div><span className="text-gray-500">Status:</span> <span className={`ml-1 px-2 py-0.5 rounded text-[10px] font-semibold ${statusColors[(viewModal.status || "draft").toLowerCase()]}`}>{(viewModal.status || "draft").toUpperCase()}</span></div>
              </div>
                ...
                <tbody>
                  {(viewModal.items || []).map((it, i) => (
                    <tr key={i} className="border-b">
                      ...
                      <td className="px-2 py-1 text-right">{(it.mrp || 0).toFixed(2)}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 z-40 bg-black/40 flex items-start justify-center pt-6 overflow-auto">
          <div className="bg-white rounded shadow-lg w-full max-w-4xl m-4" onKeyDown={handleKeyDown} tabIndex={-1}>
            <div className="flex items-center justify-between px-4 py-2 border-b bg-gray-50">
              <span className="text-sm font-semibold">New Purchase Challan</span>
            ...
            <div className="p-4 space-y-3 text-xs">
              ...
              <div>
                ...
                <div className="border rounded overflow-auto">
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="bg-gray-100 border-b">
                        <th className="px-2 py-1.5 text-left w-[20%]">Name</th>
                        <th className="px-2 py-1.5 text-left w-[10%]">Batch</th>
                        <th className="px-2 py-1.5 text-left w-[12%]">Expiry</th>
                        <th className="px-2 py-1.5 text-center w-[8%]">Qty</th>
                        <th className="px-2 py-1.5 text-right w-[10%]">Rate</th>
                        <th className="px-2 py-1.5 text-right w-[10%]">MRP</th>
                        <th className="px-2 py-1.5 text-right w-[8%]">GST%</th>
                        <th className="px-2 py-1.5 text-right w-[12%]">Amount</th>
              ...
              <div className="flex justify-end gap-2 border-t pt-3">
                ...
                >
                  {saving && <Loader2 size={12} className="animate-spin" />} Save Challan
                </button>
              </div>
            </div>
          </div>
