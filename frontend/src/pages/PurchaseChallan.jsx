...
export default function PurchaseChallan() {
  ...
  const handleSupplierSelect = (name) => {
    ...
    if (found) {
      ...
      setSupplier({ name: found.name, phone: found.phone || "", address: found.address || "",
        dlNumber: found.dlNumber || "", gstNumber: found.gstNumber || "",
        dueDate: due.toISOString().split("T")[0], transportDetails: supplier.transportDetails || "Hand Delivery",
      });
    } else {
  ...
  useEffect(() => { fetchBills(); fetchItems(); fetchSchemes(); fetchSettings(); fetchSuppliers(); fetchSalesmen(); }, []);
  ...
  const handleItemSelect = (index, searchStr) => {
    ...
    if (found) {
      ...
      updated[index] = { ...updated[index], name: found.name, batch: found.batch || "", hsn: found.hsn || "",
        ...
        amount: calculateAmount(found.rate || found.mrp, updated[index].qty, updated[index].gst),
      };
  ...
  const resetForm = () => {
    setRows([{ ...emptyRow }]); setSupplier({ name: "", phone: "", address: "", dlNumber: "", gstNumber: "", transportDetails: "Hand Delivery", dueDate: "" });
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-gray-50">
          <div className="bg-[#1b4985] text-white px-6 py-3 flex items-center justify-between shadow-md flex-shrink-0">
            <div>
              <h1 className="text-lg font-bold tracking-wide">PURCHASE CHALLANS (GOODS RECEIPT)</h1>
            ...
            <button onClick={() => setView("create")}
              ...
              <Plus className="w-3.5 h-3.5" /> New Purchase Entry
            </button>
          </div>
            ...
            <table className="w-full text-sm border-collapse">
              <thead className="bg-gray-100 border-b-2 border-gray-200 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="py-2 px-3 text-left text-xs font-semibold text-gray-500 uppercase">Supplier</th>
                      ...
                      <td className="py-1.5 px-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {(bill.status === "draft" || bill.status === "pending") && (
                            <button onClick={(e) => { e.stopPropagation(); handleApprove(bill._id || bill.id); }}
                              className="text-[10px] font-bold px-2 py-0.5 bg-green-50 text-green-700 rounded hover:bg-green-100">Approve</button>
                          )}
                          {bill.status === "approved" && (
                            <button onClick={(e) => { e.stopPropagation(); handleConvertToInvoice(bill._id || bill.id); }}
                              className="text-[10px] font-bold px-2 py-0.5 bg-purple-50 text-purple-700 rounded hover:bg-purple-100">Convert</button>
                          )}
                          {(bill.status === "draft" || bill.status === "pending") && (
                            <button onClick={(e) => { e.stopPropagation(); handleDelete(bill._id || bill.id); }}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-gray-50 relative">
          ...
          <div className="bg-[#1b4985] text-white px-6 py-2.5 flex items-center justify-between shadow-md flex-shrink-0">
            <div>
              <h1 className="text-base font-bold tracking-wide">PURCHASE ENTRY — {new Date().toLocaleDateString('en-GB')}</h1>
              <p className="text-[11px] text-blue-200 opacity-80">PURCHASE ENTRY</p>
            </div>
            ...
            <table className="w-full text-sm font-semibold border-collapse">
              <thead className="bg-[#1b4985] text-white sticky top-0 z-10">
                <tr>
                  ...
                  <th className="py-2 px-2 text-right border-r border-blue-800 w-24">P.RATE</th>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <tr key={i} onFocusCapture={() => setActiveRowIndex(i)} onClick={() => setActiveRowIndex(i)} className="border-b border-gray-200 hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="py-1.5 px-2 border-r border-gray-100">
                      <input type="number" value={row.rate} onChange={e => handleRowChange(i, "rate", e.target.value)}
                        className="w-full text-right bg-transparent outline-none font-bold text-gray-800" />
                    </td>
            ...
            <div id="invoice" className="bg-white border border-gray-200 rounded max-w-[210mm] mx-auto p-8 print:border-0 shadow-md text-xs text-gray-800">
              ...
              <div className="grid grid-cols-2 gap-6 border-b border-gray-300 pb-3 mb-4">
                <div className="pr-4 border-r border-gray-200">
                  ...
                  <p className="font-bold uppercase text-gray-900 text-sm">{activeBill.supplierName}</p>
                  <p className="text-gray-600 mt-1">{activeBill.supplierAddress || "Walk-in Supplier"}</p>
                  <p className="text-gray-600">Phone: {activeBill.supplierPhone || "N/A"}</p>
                  <p className="mt-1 font-semibold">DL No: {activeBill.supplierDl || "N/A"}</p>
                ...
                <tbody>
                  {activeBill.items?.map((item, i) => (
                    <tr key={i} className="border-b border-gray-200">
                      ...
                      <td className="border-r border-gray-300 p-2 text-right font-bold">{parseFloat(item.rate || 0).toFixed(2)}</td>
