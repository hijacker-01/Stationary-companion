...
export default function SalesChallan() {
  ...
  const fetchBills = () => {
    axios
      .get("http://localhost:5000/api/sales-challan", { headers: headers() })
  ...
  const handleSaveBill = async () => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:5000/api/sales-challan",
        payload,
        { headers: headers() },
      );
  ...
  const handleDelete = async (id) => {
    ...
    await axios.delete(`http://localhost:5000/api/sales-challan/${id}`, {
      headers: headers(),
    });
    fetchBills();
