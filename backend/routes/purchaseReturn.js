...
router.get("/", async (req, res) => {
  try {
    const returns = await PurchaseReturn.findAll({ where: branchWhere(req), order: [["createdAt", "DESC"]] });
    res.json(returns);
...
router.get("/:id", async (req, res) => {
  try {
    const ret = await PurchaseReturn.findOne({ where: branchWhere(req, { id: req.params.id }) });
...
router.delete("/:id", async (req, res) => {
  const t = await sequelize.transaction();
  try {
    const ret = await PurchaseReturn.findOne({ where: branchWhere(req, { id: req.params.id }), transaction: t });
    if (!ret) return res.status(404).json({ error: "Not found" });
    // Restore stock
